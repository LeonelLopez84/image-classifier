...
def login(username: str, password: str) -> Optional[str]:
    ...
    # 3. Preparar el payload (OAuth2 usa este formato)
    data = {
        "grant_type": "",
        "username": username,
        "password": password,
...
def predict(token: str, uploaded_file: Image) -> requests.Response:
    ...
    """
    Envía la imagen a la API para su clasificación.
    """
...
def send_feedback(
    ...
    # 4. Return the response.
    """
    Envía el feedback sobre la clasificación a la API.
    """
    url = f"{API_BASE_URL}/feedback"
    
    # 1. Datos del feedback según el esquema esperado
    data = {
        "feedback": feedback,
        "score": score,
